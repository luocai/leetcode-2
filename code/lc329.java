...
public class lc329 {
    ...
    public int dfs(int[][] matrix, int i ,int j, int[][] cache){
        ...
        if(i-1 >= 0 && matrix[i-1][j] > matrix[i][j])
            max = Math.max(dfs(matrix,i-1,j,cache)+1, max);
        
        if(i+1 < cache.length && matrix[i+1][j] > matrix[i][j])
            max = Math.max(dfs(matrix,i+1,j,cache)+1, max);
        
        if(j-1 >= 0 && matrix[i][j-1] > matrix[i][j])
            max = Math.max(dfs(matrix,i,j-1,cache)+1, max);
        
        if(j+1 < cache[0].length && matrix[i][j+1] > matrix[i][j])
            max = Math.max(dfs(matrix,i,j+1,cache)+1, max);
